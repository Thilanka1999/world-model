...
class ReducedRansac:
    def __init__(self, check_num, thres):
        super(ReducedRansac, self).__init__()
        ...
        if nonzeros_num.detach().cpu().numpy() == n:
            ...
            for i in range(b):
                ...
                if nonzero_idx.shape[0] != 0:
                    ...
                    select_idx = torch.Tensor(0, 1).to(nonzero_idx.get_device())
    ...
    def __call__(self, match, mask):
        ...
        top_ratio_match, top_ratio_mask = self.top_ratio_sample(
            match, mask, ratio=0.20
        )  # [b, 4, ratio*H*W]
        check_match, check_num = self.robust_rand_sample(
            top_ratio_match, top_ratio_mask, num=self.check_num
        )  # [b, 4, check_num]
        ...
        for i in range(b):
            f, m = cv2.findFundamentalMat(
                check_match[i, :2, :].transpose(0, 1).detach().cpu().numpy(),
                check_match[i, 2:, :].transpose(0, 1).detach().cpu().numpy(),
                cv2.FM_RANSAC,
                0.1,
                0.99,
